...
- **Phase 0 (baseline)** — 30-min live monitor (`scratchpad monitor30.py`): WS chart-feed events measured 1:1 with cycles, chart-feed latency p50 ≈ 110 ms, all 48 tags in one event under one shared timestamp, DB freshness p50 < 1 s. Also captured the failure signature this phase exists to kill: a 7-min hole (12:56→13:04 UTC) ending in an event-loop-stale process kill — persist-timeout → stall loop → wedge-watchdog suicide, all while the reader was healthy.
