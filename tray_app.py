...
class TrayApp:
    def __init__(self):
        ...
        self.plc_ip = "10.162.12.241"  # Default IP
        self.reading_interval = 1.0  # Default 1 second (1000ms)
        self.active_gateway = "allen_bradley"  # Default gateway
        self.last_active_tab = 0  # Track last active tab
        self.connection_lost = False  # Track connection loss
    ...
    def create_ui_window(self):
        ...
        siemens_ip_var = tk.StringVar(value="192.168.10.242")
        ...
        opc_ip_var = tk.StringVar(value="192.168.10.242")
        ...
        equipment_var = tk.StringVar(value="MACHINE-01")  # Default equipment name
        ...
        site_var = tk.StringVar(value="Limerick")  # Default site name
        ...
        area_var = tk.StringVar(value="LineA")  # Default area name
    ...
    def start_reading(self, root, notebook, ab_tags_var, ab_ip_var, siemens_ip_var, opc_ip_var, interval_var, ab_condition_var, ab_condition_enabled_var, equipment_var, site_var, area_var):
        ...
        # Check if reading is already in progress from a different tab
        if self.is_reading and selected_tab != self.last_active_tab:
            # Ask for confirmation to stop current reading
            result = messagebox.askyesno("Confirm Switch", 
                                        "Reading is in progress from a different tab. Do you want to stop the current reading and switch?")
            if not result:
                return  # User cancelled
            else:
        ...
        # Clear results if switching tabs (even if not reading)
